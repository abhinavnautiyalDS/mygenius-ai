...
def ask(
    ...
    try:
        # FIX: Retrieve the file path for this session if it exists
        file_path = SESSION_FILE_STORAGE.get(session_id)

        result = router.route(
            query=query,
            file_path=file_path, # Now passing the actual path!
            session_id=session_id
        )
...
async def upload_rag(
    ...
    try:
        ...
        # FIX: Save the path in our storage dictionary
        SESSION_FILE_STORAGE[session_id] = temp_file.name

        rag_agent.load_document(temp_file.name, session_id)
